 ...
 * class IndexFile. It provides an insert/delete interface.
 */
public class BTreeFile extends IndexFile implements GlobalConst {
	 ...
	 * visualization tool that shows the inner workings of the b-tree during its operations.
	 *
	 ...
	 * @throws ConvertException error when convert between revord and byte array
	...
	private KeyDataEntry _insert(KeyClass key, RID rid, PageId currentPageId)
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			KeyDataEntry undoEntry = splitIndexPage(delRid, firstRid, currentIndexPage, newIndexPage,
			...
			KeyDataEntry tmpEntry = newIndexPage.getFirst(firstRid);
			...
			if (currentLeafPage.available_space() >= BT.getKeyDataLength(key, NodeType.LEAF)) {
				...
				if (trace != null) {
					trace.writeBytes("PUTIN node " + currentPageId + lineSep);
					trace.flush();
				}
			...
			newLeafPage.setPrevPage(currentPageId); // for dbl-linked list
			...
			RID firstRid = new RID();
			KeyDataEntry undoEntry = splitLeafPage(firstRid, currentLeafPage, newLeafPage, currentPageId,
					newLeafPageId);

			if (null != undoEntry && BT.keyCompare(key, undoEntry.key) < 0) {
				// undo the final record
			...
			// fill upEntry
			KeyDataEntry tmpEntry = newLeafPage.getFirst(firstRid);
	...
	private KeyDataEntry splitIndexPage(RID delRid, RID firstRid, BTIndexPage currentIndexPage,
			BTIndexPage newIndexPage, PageId currentPageId, PageId newPageId)
			throws IOException, IteratorException, IndexInsertRecException, DeleteRecException {

		traceSplit(currentPageId, newPageId);
		...
		for (tmpEntry = newIndexPage.getFirst(firstRid);
				(currentIndexPage.available_space() > newIndexPage
	...
	private KeyDataEntry splitLeafPage(RID firstRid, BTLeafPage currentLeafPage,
			BTLeafPage newLeafPage, PageId currentPageId, PageId newPageId)
			throws IOException, IteratorException, IndexInsertRecException, DeleteRecException, LeafInsertRecException {
	...
	private void traceSplit(PageId currentPageId, PageId newIndexPageId) throws IOException {
		if (trace != null) {
			if (headerPage.get_rootId().pid != currentPageId.pid) {
				trace.writeBytes("SPLIT node " + currentPageId + " IN nodes " + currentPageId + " "
						+ newIndexPageId + lineSep);
			} else {
				trace.writeBytes("ROOTSPLIT IN nodes " + currentPageId + " " + newIndexPageId + lineSep);
			}
			trace.flush();
		}
	}
	...
	/**
	 * delete leaf entry given its <key, rid> pair. `rid' is IN the data entry; it is not the id of
	 * the data entry)
	 *
	...
	BTLeafPage findRunStart(KeyClass lo_key, RID startrid)
	...
	/**
	 * delete_leaf method will traverse the page to find the key.once the key is found , key will be
	 * deleted.if the leaf page underflows based on the given occupancy criteria , either
	 * redistribution or merge will happen to balance the Tree.If merge happens , it will return merge
	 * key which needs to be deleted in parent page or this method will return null in all other case
	 *
	 * @param currentPage the page where key is supposed to be.
	 * @param key key need to be deleted
	 * @param rid rid of the key to be deleted
	 * @param currentPageId page id of the page where key is supposed to be
	 * @param parentPageId page id of the currents's parent page
	 * @return key_class merge key which needs to be deleted in current's parent page
	 */
	private KeyClass deleteLeaf(Page currentPage, KeyClass key, RID rid, PageId currentPageId,
			PageId parentPageId)
		...
		while ((tmpEntry != null) && (BT.keyCompare(key, tmpEntry.key) >= 0)) {
			if (leafPage.delEntry(new KeyDataEntry(key, rid))) {
				...
				} else if ((PAGE_SIZE - leafPage.available_space()) < (int) (PAGE_SIZE * (PERCENTAGE
						/ 100.0f))) {
	...
	/**
	 * delete_index method will be called recursively until current page is of type index. Once
	 * delete_leaf method deletes the key and returns the merge key which needs to be deleted in the
	 * index page , delete index will delete exact merge key if it matches in index page or first key
	 * which is greater than merge key in parent index page.After key in the index page is deleted ,
	 * if page occupancy falls below the occupancy criteria , either redistribution or merge will
	 * happen to balance the Tree.If merge happens , it will return merge key which needs to be
	 * deleted in parent page will be returned or this method will return null in all other case
	 *
	 * @param currentPage the page where key is supposed to be.
	 * @param key key need to be deleted
	 * @param rid rid of the key to be deleted
	 ...
	 * @return key_class merge key which needs to be deleted in current's parent page
	 */
	private KeyClass deleteIndex(Page currentPage, KeyClass key, RID rid, PageId currentPageId,
			PageId parentPageId)
		...
		if (deleteUpEntry != null) {
			...
			if (pageIndex.getCurPage().pid == headerPage.get_rootId().pid) {
				if (pageIndex.numberOfRecords() != 0) {
					...
					BTSortedPage childPage = new BTSortedPage(pageIndex.getPrevPage(),
	...
	/**
	 * _Delete method will be recursively called until the key is found in the leaf page.once the leaf
	 * page data is redistributed or merged, recursion will unfold, by redistributing the immediate
	 * parent page until it reaches root page or currents page satisfies the page occupancy criteria.
	 *
	 * @param key key need to be deleted
	 * @param rid rid of the key to be deleted
	 * @param currentPageId page id of the page where key is supposed to be
	...
	/**
	 * This method will either redistribute or call merge method by finding the sibling of the current
	 * page and checking for the occupancy criteria.if the page is redistributed with sibling page ,
	 * all the required pages will be un pinned and method will return null
	 *
	 * @param parentPage Page object of the parent page
	 * @param currentPageId page id of the page where key is supposed to be
	 * @param parentPage page id of the currents's parent page
	 * @param key key need to be deleted
	 * @return key_class merge key which needs to be deleted in current's parent page
	 */
	private KeyClass dataRearrangement(Page currentPage, PageId currentPageId, PageId parentPage,
			KeyClass key)
		...
		if (sortPage.getType() == NodeType.INDEX) {
			...
			if (siblingDirection != 0) {
				...
				System.out.println(
						"Tried index redistributing " + currentIndexPage.getCurPage().pid + " with "
								+ siblingPageId.pid);
				redistribute = siblingPage
						.redistribute(currentIndexPage, parentIndexPage, siblingDirection, key);
			...
			if (siblingDirection != 0) {
				...
				System.out.println(
						"Tried leaf redistributing " + currentLeafPage.getCurPage().pid + " with "
								+ siblingPageId.pid);
				redistribute = siblingPage
						.redistribute(currentLeafPage, parentIndexPage, siblingDirection, key);
				if (!redistribute) {
	...
	/**
	 * merge method will merge two index/leaf pages to one and delete the other page which is empty.if
	 * the index page is merged , first key from the parent is pulled down into the left child page.
	 *
	 * @param siblingPage sibling (left/right) page object of the current page
	 * @param currentPage page id of the current page
	 * @param direction direction of the sibling to the current page
	 * @param parentIndexPage page object of the current's parent page
	 * @return key_class merge key which needs to be deleted in current's parent page
	 */
	private KeyClass merge(Object siblingPage, Object currentPage, int direction,
			BTIndexPage parentIndexPage)
			throws LeafInsertRecException, DeleteRecException, IteratorException,
		...
		if (siblingPage instanceof BTLeafPage && currentPage instanceof BTLeafPage) {
			...
			for (KeyDataEntry tmpEntry = rightChild.getFirst(mergeRid); tmpEntry != null;
					tmpEntry = rightChild
							.getFirst(mergeRid)) {
	...
	/**
	 * Since merge will delete the page , this method will arrange pointers between left page and
	 * right page of the deleted page.
	 *
	 * @param leftChild page object of the parent page's left child
	 * @param rightChild page object of the parents page's right child
	 */
	...
	/**
	 * create a scan with given keys Cases: (1) lo_key = null, hi_key = null scan the whole index (2)
	 * lo_key = null, hi_key!= null range scan from min to the hi_key (3) lo_key!= null, hi_key = null
	 * range scan from the lo_key to max (4) lo_key!= null, hi_key!= null, lo_key = hi_key exact match
	 * ( might not unique) (5) lo_key!= null, hi_key!= null, lo_key < hi_key range scan from lo_key to
	 * hi_key
	 *
	...
	void trace_children(PageId id)
		...
		if (trace != null) {
			...
			if (sortedPage.getType() == NodeType.INDEX) {
				...
				for (entry = indexPage.getFirst(metaRid); entry != null;
